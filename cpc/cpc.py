# Based on this code: https://github.com/unixb0y/CPY-CC1101
...
class CC1101:
    ...
    def readBurst(self, start_address, length):
        ...
        with self.device as d:
            print("Reading burst from address %d" % start_address)
            print("Reading %d bytes" % length)
            print("Reading from addresses: %s" % databuffer)
    ...
    def receiveData2(self, length):
        ...
        data_len = length
        data = self.readBurst(RXFIFO, data_len)
        dataStr = ''.join(["{0:08b}".format(byte) for byte in data])

        # Determine the start of the actual message
        # This depends on how the sendData function is structured.
        # For this example, let's assume the first 8 bytes (64 bits) are not part of the message
        actual_data_start = 8
        actual_dataStr = dataStr[actual_data_start:]

        # Convert binary data to string
        message = binary_to_string(actual_dataStr)
    ...
    def sendData2(self, input_string, syncword):
        # Convert input string to binary
        bitstring = ''.join(char_to_bin(char) for char in input_string)
...
def char_to_bin(char):
    ...
    return '0' * (8 - len(binary_representation)) + binary_representation
